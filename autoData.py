...
C_ADDR = '0xf5cb8dfce2fb230d1842264a96c4f1ddb469c399'
#C_ADDR = Web3.toChecksumAddress(C_ADDR_1)
submit_block = True
web3=Web3(IPCProvider("/home/pi/1sec_floodblock/n1/geth.ipc"))
...
if __name__ == '__main__':
                ...
                while True:
                        ...
                        if fencetest == True:
                                ...
                                if blockno % 2!=0:
                                        ...
                                        setpump=int(testcontract.call().setpump())
                                        print("SetPump: %i" % bool(setpump))
                                        act_pump = bool(testcontract.transact({'from':web3.eth.coinbase}).calcflood())
                                        act_pump = int(testcontract.call().setpump())
                                        if act_pump == True:
                                ...
                                if blockno % 2==0:
                                        ...
                                        setpump=int(testcontract.call().setpump())
                                        print("SetPump: %i" % bool(setpump))
                                        testcontract.transact({'from': web3.eth.coinbase, 'gas': 200000, 'value': int(50)}).SendWL()
                                        print ("no flood ")
                                        while blockno >= int(web3.eth.blockNumber):
                                                 count=count+1
                                        blockno = (web3.eth.blockNumber)
        ...
        # reset by pressing CTRL + C
                ...
                for index in range(len(a)):
                       np.savetxt("delay.txt", data, fmt=['%d', '%d'])
                print(".")
